...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        window?.tintColor = UIColor.Bitrise.brightPurple
        
        let split = UIKitSplitNavigator()
        let masterNav = UIKitNavigator()
        masterNav.setBitriseAppearance()
        let detailNav = UIKitNavigator()
        split.viewControllers = [masterNav, detailNav]
        
        appController = AppController(with: split)
        appController?.start()
